...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        http
                ...
                .antMatchers("/micfin/login").permitAll()
                .antMatchers("/micfin/signup").permitAll()
                .antMatchers("/micfin/swagger-ui.html").permitAll()
                .antMatchers("/micfin/api/**").hasAuthority("ADMIN").anyRequest()
                .authenticated().and().csrf().disable().formLogin().successHandler(customizeAuthenticationSuccessHandler)
                .loginPage("/micfin/login").failureUrl("/micfin/login?error=true")
